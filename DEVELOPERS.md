...
**The video overlay has one source of truth.** `frontend/src/lib/video-overlay.ts` answers what the overlay shows at a moment: which boxes, where they are between the frames the detector sampled, the dimming of unselected tracks, and the trail behind the selected animal. `VideoPlayer` holds two renderers over that one answer, SVG for the screen and canvas for the annotated MP4 it records, and neither decides anything. They used to decide separately: the recorder carried its own copy of the frame lookup and the fade, and had already drifted, never dimming the other tracks. If you change what appears on a playing video, change the module.
