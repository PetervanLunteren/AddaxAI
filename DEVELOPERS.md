...
1. **MD output**: MegaDetector always runs at `MD_OUTPUT_CONFIDENCE_THRESHOLD = 0.01`, passed as `--threshold` (images) and as the tracker's low threshold `--track_low_thresh` (videos, see "Every video is tracked": a box that low is stored only when it extends a track that started at the default counting threshold). Everything at or above it is stored: raw results.json, the database, and `addaxai-recognitions.json`, which is the one export that still carries every stored box on every frame.
...
3. **`Project.counting_threshold`** (default 0.5): the counting/visualization filter described below. A folder run gets the same default and the same meaning; it is not pinned to the classification gate (that pinning was removed, see the comment in `routers/folder_runs.py`). The default was 0.2 until 2026-09-14, MegaDetector's own *display* default for v5 inherited as the counting floor; the tracking benchmark (`TRACKING_BENCHMARK_RESULTS.md`) measured MaxN against human counts on three datasets and found 0.2 overcounting everywhere, with the exact-count peak flat between 0.4 and 0.6 for MD5A (PanAf500 video 57% to 69%, SA-FARI video 66% to 72%, iWildCam photo sequences 64% to 71%). It is one number for every detector by decision; the underwater detectors were not measured, and a sweep on labelled BRUV clips is the open follow-up. Projects keep the value they were created with.
...
**Both rules live in `passesDrawFilter`, not in `shouldDrawBbox` itself.** `VideoPlayer` draws every frame's boxes over the real video on purpose, so it cannot use `shouldDrawBbox` (the frame gate would blank it), and it used to carry its own inline `confidence < threshold` instead. Neither rule reached it, and the result was one event modal disagreeing with itself: in the Counts event view a box a human confirmed below the threshold drew in frame mode and vanished on play, while a box they rejected did the reverse. If a new surface needs the rules without the frame gate, call `passesDrawFilter`; do not inline the comparison again.
...
**Scoped to the file's cards.** `on_visible_frame_of(file)`: every box
of a photo, and for a video the representative box of every track,
which `expand_to_tracks` then carries to every box of that track. That
is the surface the Files viewer shows as bars and the surface
`recompute_file_verified` reads, so a sign-off can never be flipped
back by the next rollup. One Enter on a 90-minute clip therefore signs
off every animal in it; the viewer says how many. A box that is not a
card (an untracked row of a pre-tracking run) is neither rejected nor
verified.
...
**For a video, "its detections" means its cards.** The module itself is frame-blind: every caller passes it the file's *visible surface* first, via `on_visible_frame()` / `on_visible_frame_of()` in a query or `visible_detections(file, dets)` on a list already in memory. For a video that surface is one box per track, the representative box, whatever frame it sits on; a tracked clip is therefore named by its strongest track (a 90-minute drop with one 0.95 shark and a 0.3 fish reads `elasmobranch`), the same rule as a photo with several animals. The cover frame plays no part. A video with no tracks reads `blank`, which is the honest answer: no card in the Labels grid, no MaxN count, no crop. Its boxes, if any, are still in the recognition JSON. Migration `6f7a8b9c0d1e` backfilled existing videos under the older best-frame rule; `f6a7b8c9d0e2` then gave every such visible box its own one-frame track, so the rule needs no legacy branch.
...
**Do not add a fifth category map.** There were four, all subtly different, and one Pydantic `Literal` that rejected any other detector's output at the schema boundary before the code that would have handled it ran. They are now one function plus the Camtrap boundary. If you find yourself writing `if category == "animal"`, you almost certainly mean **is this wildlife**, and there is one answer to that.

**Every detector declares its classes and its domain in the catalog.** `ModelManifest.domain` is `camera_trap` or `underwater`; it decides the track post-filter's values (see "Every video is tracked"), and the catalog test requires it on every detector like `classes`. Classifiers declare it too (all `camera_trap` today); embedders work on any crop and declare none. `ModelManifest.classes` is on all eleven: animal/person/vehicle for the MegaDetectors, `elasmobranch` for SharkTrack, `fish` for the Community Fish Detector variants. It exists because nothing else in the app knows what a detector finds until a run has produced it, and the label picker needs that before the first analysis. **Lookup and display only.** Nothing in the detection or ingest path reads it: a run's own JSON carries the authoritative `detection_categories`, and `json_pipeline` refuses an id that map never declared, so a stale entry can only show a wrong option in a picker and can never mislabel a stored box. `GET /api/projects/{id}/categories` is the only reader; installed manifests pick the field up on the next catalog sync, since `write_manifest` refreshes them in place. The values were read off the weights themselves and are pinned by `tests/ml/test_catalog_schema.py`, which also enforces that every detector has them (the schema cannot: it is shared with classifiers and embedders, which have none).

**The data type is the detector's domain, and nothing else.** The setup forms (folder-run setup, create project, project settings) open with a "Data type" toggle, camera trap or underwater, that offers only the detectors and classifiers of that type and swaps any model that does not fit (`applyDataType` in `frontend/src/lib/data-type.ts`, one implementation for all of them; the duplicate dialog filters by the source's detector). There is no project column: a project's type is read from its detector, so the two cannot disagree, and the only stored value is the user's last choice in the last-used settings in localStorage, which is where a *new* run or project starts. An existing project or resumed run always shows its own type, or its picker would hold a detector the filtered list does not contain. A stored column is the right move only once something needs the type apart from the detector, such as per-drop rates instead of trap nights; it would be backfilled from the detector's domain. The filter is not cosmetic: classification runs on every box that is not a person or vehicle and a top-1 "blank" skips the box at ingest, so a camera trap classifier on SharkTrack's boxes drops sharks silently. `_refuse_mixed_domains` in `routers/projects.py` therefore refuses that pair with a 400 on create, update and duplicate, for every path the forms do not cover (stale last-used settings, the API). It skips a model that is not installed or declares no domain, so it can never block a valid project. A duplicate always keeps the source's detector for the same reason, settings copied or not. Pinned by `tests/api/test_project_data_type.py` and `test_duplicate_without_settings_keeps_the_detector`.

**A detector the package cannot name also declares `class_mapping`**, which answers a different question: not what it finds but **which id it emits for each class**. `{"0": "elasmobranch"}` on SharkTrack, and on nothing else, since the MegaDetectors' classes the package knows and RF-DETR carries its names on the detector object. Unlike `classes` this one **is** read by the detection path: it is handed over as `--class_mapping_filename`, so it decides the ids the run reports. The ids are the model's own, from zero. Never give one to a MegaDetector: the flag also switches the package to native class indices, which would turn its output categories from `1/2/3` into `0/1/2` and disagree with every stored `results.json` and with Timelapse. `ml/inference/class_mapping.py` writes the file for both the image and the video path, so those two cannot drift, and the catalog test pins the mapping's names against `classes` in id order so those two cannot either.

The two overlap in their names, and that is deliberate rather than duplication. They are asked at different moments by different code, a mapping is needed by one model in eleven, and a single field would have to mean both "the list a picker shows" and "the ids inference reports". One rule you can state in a sentence, with a test holding the halves together, beats a field whose shape changes per model.

**Person and vehicle are not wildlife. Every other category is.** `is_wildlife(name)` and `is_wildlife_category(column)` in `ml/label_exclusion.py`, two lanes over one `NON_WILDLIFE_CATEGORIES` frozenset, the same shape as `is_a_real_detection` beside them and pinned by the same kind of parity test. A closed list of what is *not* wildlife, because a detector we have never seen ships classes we cannot enumerate and counting one of them as wildlife is the harmless direction; if one ever emits a `boat`, that is one more entry in one frozenset.

Until 2026-09-11 about twenty places asked `category == "animal"` instead, and a shark was therefore neither wildlife nor a person: `GET /deployments/{id}/info` reported `animal: 0` and no top species beside eight observations, the dashboard collapsed every marine species into one `elasmobranch` bar carrying another species' taxonomy id, `_species_label` exported the category in place of the species, and Camtrap DP wrote 701 media observations with no `scientificName` while the event row for the same species carried one. The three remaining literal comparisons are deliberate: the anonymise blur asks for `person`, Camtrap DP translates `person` to its fixed `human` vocabulary, and the deployment and site counters count persons and vehicles as themselves.
...
> Sort the classes present in the project by taxonomy (class, order, family, genus, species, variant, name), then give rank `i` the palette entry `i mod 18`. The palette is ordered farthest-first, so consecutive entries contrast most, and consecutive ranks are the classes most likely to be confused for each other.

A class is whatever names the box: its species when a classifier named one, the detector's category when none did. Which model it came from makes no difference, so "animal", "person", "vehicle" and another detector's "elasmobranch" take palette slots exactly like a species. A category carries no taxonomy, so those sort together in front of the species, by name.

"Present" is `present_label_rows` in `crud/event.py` for anything with a taxonomy row, and `present_category_rows` beside it for a category that has none, both the same scope as the label filter, so a class can never be filterable without a colour or the other way round. A category is only ranked when boxes still show it: once every elasmobranch box carries a species, the category is never drawn and spends no slot.
...
**Consequences to know.** Colours are per project, and a class that first appears through a relabel shifts the classes after it by one slot, so `invalidateLabelQueries` refetches the map together with the label tree on every relabel. A key the map does not know renders neutral grey on screen (visible, never plausible) and a deterministic palette pick in the export, which can only happen for something that passes the media threshold but not the counting threshold. Class 19 onwards wraps; that pairs a class with one eighteen ranks away, which is almost never a relative.

**Eighteen, not twelve.** The palette grew on 2026-09-11, keeping the original twelve exactly where they were so no project's colours moved. The limit is not the number of colours the eye can tell apart but the palette's own recipe, a narrow chroma band over a few lightness steps, which is what stops it looking like a box of highlighters. Inside that recipe eighteen is free: every added colour is at least 16 CIEDE2000 from everything already there, so the closest pair is the same 16 it was at twelve, and the worst step between neighbouring ranks improved from 16 to 30 because the weak wrap from the last colour back to the first went away. Twenty would cost a little (closest pair 15.1) and rebuilding all of them from scratch scored worse on both counts. The real weak point is colour vision deficiency: two of the original twelve are already indistinguishable under deuteranopia, more colours makes that worse, and the pill's species name is what carries the meaning.

**There is no category table any more.** Until 2026-09-10 the three MegaDetector categories held fixed colours (animal teal, person orange, vehicle light teal) and took no slot, and the palette was generated to avoid those three. That special case existed in three places and they had drifted: `label_colors.py` and the frontend gave any other detector's category the animal teal, while the export's own table gave it the brand red, so every unclassified box of a marine run drew teal on screen and red in the JPEG, in the same red as palette rank 0. One rule for every class removes all three copies. MegaDetector's categories still get a `__builtin__` taxonomy row at ingest (`taxonomy_db.ensure_builtin_labels`), which is now only how they reach the map, not a different colour for them.
...
**What restarts on a resume.** Everything after detection: classification is a one-shot subprocess with temp in/out files (no partial output exists), cover-frame selection and embeddings are cheap next to detection. Videos restart fully: the tracking script writes no checkpoint. A per-video resume (write each video's boxes and crops as it finishes, skip finished files on restart) is the first follow-up if long runs show it is needed.
...
| the classifier with crops (11 to 51 frames) | many, close together | `iter_wanted_frames`, walking |
| the filmstrip (9) | few, far apart | `read_wanted_frames`: a seek each, then one walk for what a seek could not verify |
| any frame a person looks at (`/image?frame=N`, a card cut on request) | 1, any | `services/video_frame_service.py`: an ffmpeg keyframe seek and decode, hardware where there is one, a 64-entry memory cache, nothing on disk |
...
**The scoring decides only the picture.** `CONFIDENCE_THRESHOLD = 0.3` in `scoring.py` picks the frame with the most confident boxes as the cover; since every video is tracked, `File.observation_type` comes from the tracks and does not move with this floor.
...
Every video box belongs to a track, and the track has one card: the box on `Track.representative_frame_number`, the frame of its highest-confidence box. That makes one rule non-negotiable:

> a video detection is visible when `Detection.frame_number == Track.representative_frame_number` of its track

Break it and nothing errors. Cutting a card at a bbox from a different moment returns a perfectly valid JPEG of the wrong place. On a clip of a person walking, 31 of 32 grid tiles were pictures of the background they had already left. The bug survived for months because a slow subject hides it completely: successive bboxes overlap, so the crops still contain the animal, just shifted, and they look right. Only a moving subject exposes it. If you are ever unsure whether a surface honours this, test it with a clip of something walking across the frame, never with a browsing deer.

`ml/detection_visibility.py` holds the rule (`on_visible_frame`, `on_visible_frame_of`, `visible_detections`, one parity test), and these carry a hand copy:

| Module | How |
|--------|-----|
| `services/crop_service.py` | `_resolve_source`: the track's crop, else the cover when the card sits on it, else the frame decoded on request; `None` only for a box with no track or no frame. A sibling frame does get a crop, cut from its own decoded frame at its own box, because the opened track shows them (see below) |
| `ml/inference/similarity_script.py` | `_COMMON_JOINS` gates all three grid queries |
| `ml/embedding_utils.py` | `video_pixels_for`: the crop or the cover; a card with neither is skipped (the embedder cannot decode) |
| `api/crud/event_observation.py` | `any_representative` admits a species to the counts |
| `frontend/src/lib/detection-utils.ts` | `shouldDrawBbox` draws the boxes of the frame on screen |

There is no verified-anywhere exception: a verdict on the card reaches every box of the track through `expand_to_tracks`, so the card always carries it, and letting verified boxes through on other frames would turn every frame of a verified track into a grid row and an export row. An untracked video box (an unverified off-cover row of a run analysed before tracking became the standard) is visible nowhere, kept only so a reprocess of that run still matches its JSON.

**The other boxes of a track are not junk, and are not deleted.** Their per-frame classifications are exactly what smoothing and taxonomic rollup consume, and they disagree a lot: one raccoon walking through a clip was read as northern raccoon, american badger, american badger, blank, virginia opossum, northern raccoon, blank on seven consecutive frames. Rollup turning that into one label is the system working. They draw in `VideoPlayer`, which has the real frames, they count in MaxN, and they are in the recognition JSON with their track.

## Every video is tracked

Every video goes through `ml/inference/tracking_script.py`: the detector on every sampled frame (`Project.video_fps`, default 3 since 2026-09-14: the benchmark's peak with the post-filter on, and SharkTrack's own rate; it was 2), BoT-SORT linking the boxes into tracks, one `tracks` row per animal, person or vehicle followed, `track_id` on every box. There is no switch. The Labels page reviews one card per track; the Counts page counts MaxN across every frame; the Files viewer shows a clip as its cover with its tracks as bars. The design, the decisions and the comparison with other platforms are in `MARINE_VIDEOS_DESIGN.md` and the plan of 2026-09-08.
...
**Boxes are drawn on photos only.** `POST /api/detections` refuses a video with a 400 that names the Counts page. A hand-drawn box can only land on the frame the person is looking at, and on an hour of footage sampled at 3 fps that is one frame in ten thousand, so it could never be how an animal the detector missed gets recorded: the count is, on the Counts page, where a human number replaces the AI's MaxN everywhere. What this buys is that the video viewer holds no edit mode, so `drawMode`, the crosshair, the D key and the Default label card are all photo-only, and there is no rule about which frame a clip may be drawn on. The one-frame track itself stays: migration `f6a7b8c9d0e2` makes them out of legacy boxes, and released databases hold boxes drawn by hand before this. `delete_detection` still drops a track its last box leaves empty, three lines that stop a direct API delete orphaning a row.

**Opening a track.** A badge on a video card in the Detections grid says how many frames the tracker followed the animal for; clicking it swaps the grid for those frames, and a verdict there acts on what is selected rather than on the animal. That is what puts a merged track right: relabel the half that followed a different animal and every count is correct, because MaxN reads what is in each frame and not what the tracker believed. Split is still unbuilt and this is why it is not urgent.

Three things make it work. `DetectionSummary` carries `track_id` and `track_frames` (`similarity_script.py`: the SELECT list, `_row_to_meta` **and** `_build_summary`, all three, which is the step that file has skipped before). `GET /api/tracks/{id}/detections` returns one track's boxes, gated at `effective_floor(counting_threshold, min_confidence)` and the literal slider, the grid's own confidence scope: gating at the project threshold alone opens an empty track under a card the person can see, because the slider digs *down*. And `crop_service._resolve_source` grows a rung for a sibling frame. That rung must never reach `video_pixels_for`, which returns the track's stored crop whenever there is one, so every frame of the animal would render one picture that looks entirely plausible and is wrong.

**The badge counts frames tracked, not cards you get.** The tracker keeps boxes down to the storage floor while the grid shows from the counting threshold, so a 40-frame track routinely opens to nine cards. `OpenTrackBar` says so in words. Making the two agree would mean a second hand-written copy of the confidence rule inside the subprocess SQL, plus a parity test to keep it honest, to buy a number that is less informative: how long an animal was on screen is a fact about the animal.

**A card verdict carries its label.** Confirming an animal applies that species to every box of its track, not only `verified`. Until 2026-09-11 verify set the flag and left each box's own label, so a red deer card whose track held two frames the classifier read as roe deer stored those two as human-verified roe deer; verified rows are skipped by smoothing and rollup, so the misread was frozen there, and it reached the recognition JSON and the MaxN gate as a real sighting. `apply_card_labels` in `crud/detection.py` is the one implementation, called by verify, bulk verify and the file sign-off. **It skips a box whose `classification_method` is `"human"`**: opening a track and relabelling three frames is how a merged track is put right, so a later verdict on the card fills in the frames nobody touched and leaves the ones they did. A sign-off verifies without touching labels, so it can never be mistaken for one. `original_label` is untouched, so revert-to-original still hands the machine's per-frame labels back; unverify only clears the flag.

MaxN's video gate follows from that: a species counts when it has a card **or** when a person put that label on a frame themselves. The second half is what makes opening a track work at all, since the card keeps the old species and the relabelled frames otherwise lived in the database and in no count. It cannot admit classifier noise, because a card verdict normalises the track before anything is verified.

**The cascade opt-out.** `expand_tracks` on bulk-verify, bulk-relabel and bulk-revert-to-original, default true. Only the opened track sends false. It is a parameter rather than a narrower `expand_to_tracks` because that function does *not* only receive cards: `FileDetailModal` and `EventDetailModal` both send the box on the frame currently on screen (`shouldDrawBbox` resolves to the shown frame, and Tab and a canvas click select it), so narrowing the rule globally would stop a Files-viewer Enter from signing off the animal. Defaulting to true also means a caller that forgets it cascades rather than under-applies, which is the safer direction. `tests/api/test_track_cascade.py` pins both halves.

**Decoding is bounded.** Opening a track makes `decode_frame_jpeg` hot: one ffmpeg process per visible frame, and the endpoints asking for them are sync `def`, so they run in Starlette's threadpool. `MAX_CONCURRENT_DECODES = 4` keeps video decoding from owning that pool while every other request queues behind it, and `FRAME_CACHE_SIZE = 256` stops a long track evicting itself as you scroll. If a run still stalls, the next moves are to release the DB session before decoding (a decode currently holds one of the pool's fifteen connections) and a per-key lock so two requests for one frame spawn one process. Neither is built.
...
**The track holds no label and no verdict.** `tracks` is one row per animal (`file_id`, `track_key` from the JSON, frame span, count, max confidence, representative frame, `crop_path`); `Detection.track_id` points at it, `ON DELETE SET NULL`. The label is the label of the boxes and the verdict is `verified` on them, one home for each fact, so every per-frame query (MaxN, exports, the video overlay) keeps working. A verdict on the card reaches the animal: `in_expanded_tracks` in `crud/detection.py` turns the ids of a request into a criterion matching every box sharing a track, and verify, bulk verify, relabel, X, dismiss, undo and the file sign-off all go through it. A criterion rather than a list of ids on purpose: one card of an 81-minute clip expands to every box of its track and a sign-off to 12,731 of them, while SQLite's default ceiling is 32,766 bind parameters, so materialising the expansion put a long enough drop one clip away from failing the whole verdict with "too many SQL variables". The bookkeeping that follows (`recompute_file_verified_for_detections`, the observation-type refresh, `_recalculate_max_n`) is handed the request's own ids instead, which is enough because every sibling sits in the card's own file. No foreign key to a representative detection, so there is no cycle between the two tables. The purge deletes `tracks` after `detections` and before `files`; a deployment split and a relink rewrite `crop_path` beside `best_frame_path`.

**Seeing a track.** The card modal draws the animal's path on its frame (`trackPath` in `lib/track-utils.ts`) and, under the frame line, where in the clip it was (`TrackSpan`: a bar over `File.duration_seconds`, which the ingest stores from the last processed frame at the frame rate; text only for a file analysed before that was stored). "Open in files view" hands the track along as `lbl_track`. The timeline is `ClipTimeline` (`TrackTimeline.tsx`), mounted by `VideoPlayer` in video mode and by both modals in frame mode: one group per species and, inside a group, as many rows as its animals need to sit beside each other (`packTrackRows` in `lib/track-utils.ts`, one bar per animal in the species colour, solid when every box is verified, hatched while unverified, hollow when rejected), the species named on the group's first row, a playhead, MaxN markers from the Counts modal alone. Rows past the cap of six scroll, and selecting an animal scrolls its row into view. A bar click selects the track's card and shows its frame through `?frame=`; the lane shows any frame; P hands the frame on screen to the player. `cardBoxes` in `lib/track-utils.ts` is what "every detection in the file" means on the Files tab: the tile chips, the viewer's actions with nothing selected, the bulk bar and the Counts modal's landing file read it.

**Packing, and why the timeline zooms.** A species used to be one row with every one of its tracks drawn inside it. That is fine for three deer and useless for a shark drop: the bars sat on top of each other and a click landed on whichever painted last. Two tracks only need separate rows when they overlap in time, so `packTrackRows` puts each one in the first row whose last track ends before it starts, else in a new row. Walking them in start order (which `trackRows` already sorts for) makes that greedy pass land on the fewest rows there can be, the most animals of that species ever on screen at once: 702 sharks over 81 minutes need seven rows.

Packing only fixes the vertical collision. Measured on `MMTF_005_manysharks.MP4`, the median track is 0.97 px wide on an 880 px lane, the longest in the whole clip is 5.6 px, and the busiest 90 seconds is 16 px of lane holding 39 animals, so no minimum bar width can make a long clip clickable (39 bars at 6 px is 234 px of bar in 16 px of lane). Hence the zoom slider: each step halves the visible span and the window stays centred on the playhead, clamped at the clip's ends. The playhead is therefore the scroll position, so clicking in the lane pans and playback scrolls the window along, and there is no second piece of state to keep in step with it. The slider is not rendered at all when the clip is too short to zoom into. Bars outside the window are not rendered, so zooming in costs less than the overview does.

**The video overlay has one source of truth.** `frontend/src/lib/video-overlay.ts` answers what the overlay shows at a moment: which boxes, where they are between the frames the detector sampled, the dimming of unselected tracks, and the trail behind each animal. `VideoPlayer` holds two renderers over that one answer, SVG for the screen and canvas for the annotated MP4 it records, and neither decides anything. They used to decide separately: the recorder carried its own copy of the frame lookup and the fade, and had already drifted, never dimming the other tracks. If you change what appears on a playing video, change the module.

**Boxes move between samples; nothing is invented across a gap.** One rule covers every case: a sampled box is drawn for one sampling step starting at its own frame, gliding to the next sample when that sample is one step away and holding its position when it is not. So an animal never glides through a stretch the detector could not see, nothing is drawn before a track's first sample or more than one step past its last, and a track ends by stopping. The old hold-and-fade (`HOLD_FRAMES`, `FADE_FRAMES`) is gone with the boxes-vanish-between-samples problem it covered for; it drew the previous sample at the previous sample's position for about a second, which on a fast animal was simply the wrong place.

The sampling step is measured from the file's own frame numbers, the smallest positive gap between samples, not taken from `Project.video_fps`: a clip analysed before that setting last changed keeps its own older spacing, and the file's numbers cannot disagree with the file. A track's own boxes are a subset of the sampled frames, since detection fails on some, which is why the gap clause exists at all. `FRAME_EPSILON` covers the float error in `currentTime * frameRate`, without which seeking to a sample that follows a gap landed a hair short of it and drew nothing.

**An interpolated box is a position the detector never reported.** It is display only: never stored, never counted, never in a data export. It does appear in the recorded MP4, because that file is meant to match what you reviewed, so that video is the one artefact where a drawn box is not a detection.

**A stored box trails the animal a little, and that is BoT-SORT.** What the tracking script writes is the tracker's own row, and ultralytics reports the Kalman state (`STrack.result` reads `self.mean[:4]`), not the detection it matched. Measured against the detector run frame by frame on `MMTF_005_manysharks.MP4`, a stored box sits where the animal was 2 to 4 native frames earlier, about 5% of the animal's length at the median swimming speed and 11% for the fastest tracks; it is what makes a shark's nose sit on the front edge with a gap behind the tail. The overlay is not the cause: the offset is already there on the sampled frames, where the stored box is drawn verbatim. The track card is cut from the same box, so it is off-centre by the same fraction.

Using the matched detection instead was investigated and deliberately left alone. The tracker's `idx` column is not an index into our detection array: ultralytics appends it inside `init_track`, after the detections are split by score into the two association stages, so it restarts per subset. Recovering the raw box means reproducing that split from `track_high_thresh` and guarding on the score, which couples us to private ultralytics internals that can change on any version bump and would then put boxes on the wrong animal with nothing to warn us. Re-matching by IoU in our own code is not an option either: `MATCH_THRESH` is SharkTrack's 0.97, so the tracker accepts matches down to about 0.03 IoU and on exactly the fast animals the two boxes barely overlap. Counts, tracks and identities are unaffected by any of this, so it stays as it is.

The overlay therefore runs off fractional time while the timeline still runs off whole frames, so the player re-renders at animation-frame rate; `ClipTimeline` is memoised and each label pill is measured once per render rather than twice.

**MaxN tie-break.** `calculate_max_n_for_event` orders its per-frame groups by frame, so among equal counts the first frame wins, EventMeasure's default. The Camtrap DP event row says so in its tags.

**`detections.track_id` was added with raw DDL.** Alembic refuses a foreign key on an added column in SQLite and points at batch mode, which would rebuild the whole detections table for one nullable column. SQLite accepts `ADD COLUMN ... REFERENCES ... ON DELETE SET NULL` and enforces it, but the SQLAlchemy inspector parses the stored CREATE TABLE and drops the action of a column-level clause, so `schema_problems()` now reads foreign key actions from `PRAGMA foreign_key_list`, which is what the database enforces. `d4e5f6a7b8c0` is the migration.

**The tracker script.** `ml/inference/tracking_script.py` is the one video detection path (MegaDetector's `process_video` and its Windows crash retry are gone): every `stride`-th source frame with `stride = round(native_fps / fps)`, numbered by absolute source index as `process_video` numbered them, the detector loaded by the megadetector package, BoT-SORT over the sampled frames with SharkTrack's association parameters (match 0.97, buffer two seconds scaled to the fps, no score fusion, no ReID) and the app's own floors on the command line: a track starts at `DEFAULT_COUNTING_THRESHOLD` and keeps boxes down to `MD_OUTPUT_CONFIDENCE_THRESHOLD`, from `app/core/confidence.py` through `_build_tracking_cmd`, so the script holds no copy that could drift (`test_tracking_cmd_carries_the_apps_confidence_floors`). SharkTrack's false-positive filter (a track shorter than one second or nearly static is dropped unless its best box is confident enough) runs for every detector; its two numbers come from the detector's catalog `domain` through `app/ml/track_filter.py`: SharkTrack's own 0.08 of the frame and 0.7 under water, where a static box is almost always false, and 0.06 and 0.5 on a camera trap, where a still animal is ordinary. Measured on 2026-09-14 (`TRACKING_BENCHMARK_RESULTS.md`): with no filter the app's track counts were 49% exact on held-out PanAf clips at 3 fps; SharkTrack's values reach 70% but empty 9% of the clips of a resting animal (the track is deleted, the clip is filed blank, nothing is reviewed); 0.06 / 0.5 reaches 65% and empties 4%, against 2% with no filter. Until then the filter ran only for the four underwater detectors, behind a boolean `track_filter` flag that the domain field replaced. The results JSON `info` records the floors and the two filter values. The sampling is not `process_video`'s: it turned `--time_sample 1/fps` into `int(seconds * native_fps)` and so took every 9th frame of a 29.97 fps clip at 3 fps (3.33 a second), where `round` takes every 10th. A project analysed before tracking has a denser `frames_processed` than one analysed after; nothing reads the stride back, so both ingest the same way.

**It decodes through an ffmpeg pipe, not MegaDetector's `video_utils`.** The script asks ffmpeg for every stride-th source frame (`select=not(mod(n,stride))`, `-fps_mode passthrough`, so output frame `i` is source frame `i * stride`) at most 1920 px on the long edge as raw BGR (the cover's cap, so a track's crop matches a card cut from the cover). Every box is normalised, so the decode size never reaches the database. ffmpeg ships in the envs as a conda-forge package like exiftool, pinned to 9.0.1 because the script depends on its flags, and is resolved from the detector's env by `utils/ffmpeg_bin.py`, PATH as the dev fallback. OpenCV still reads the header (fps, frame count, size).

Using Dan's `run_callback_on_frames` instead was measured on 2026-09-13 (M1 Pro, MPS, SharkTrack at 3 fps, whole files), before deciding to keep the pipe:

| | 81 min 1080p drop | 65 min 4K drop | CPU cores |
|---|---|---|---|
| `video_utils` as is | 17.5 min | 26.1 min | 2.4 to 3.5 |
| `video_utils` in a decode thread, `grab()` for skipped frames | 11.9 min | 14.7 min | 3.2 to 4.6 |
| ffmpeg pipe | 12.6 min | 12.4 min | 0.9 |

The pipe wins because ffmpeg is a separate process that decodes the next frame while the detector works on this one; `video_utils` decodes in the detector's thread and `read()`s every frame it then skips. Given a thread and `grab()` (neither is in the package) it matches the pipe at 1080p and stays 18% slower at 4K. What users see is the same: MaxN identical in every comparison, box counts within 1% (14,399 against 14,253 on the 1080p drop), identical frame numbers over 37,260 frames, and the same result on rotated, variable frame rate, truncated, MPEG-4 part 2 and 10-bit HEVC clips. Two differences favour the pipe: colours match a BT.709 reference (48 to 58 dB, where OpenCV's conversion is off by up to 4.8/255), and it decodes the Bushnell AVIs `video_utils` corrupts (below). A file that will not open raises `AttributeError` inside `run_callback_on_frames`; the script writes a failure entry for it before decoding. In lines the choice is small either way: the pipe is ~60 lines of the script, `video_utils` with its thread would be ~45, and the detector, tracker, filter, crops and JSON (~580 lines) are ours under both. ffmpeg stays in the app regardless, for the viewer's frames. **Only boxes of a surviving track are written**, so the tracker's thresholds are the storage floor and a 90-minute reef video does not store hundreds of thousands of noise rows. One loader, the megadetector package's `load_detector`, for every detector (see "Every detector loads through the megadetector package" below). The script runs with `python -P`: its own directory holds the app's `megadetector.py` wrapper, which Python would otherwise find before the package; it loads the siblings `video_iter`, `scoring` and `crop_box` by file path for the same reason. It prints the `PTDetector using device` line the app reads its compute device from. Progress is tqdm over every sampled frame of the run, which `_stream_process` already parses; the per-video log line says "of", not "/", because the parser reads `n/N` off any line.

### Every detector loads through the megadetector package

There is one detection runtime. `load_detector` reads MegaDetector `.pt` files, RF-DETR `.pth` files (the Community Fish Detector) and plain ultralytics checkpoints such as SharkTrack, on both the image path (`run_detector_batch`) and the video path (`tracking_script.py`). Until 2026-09-11 SharkTrack had a second loader of its own, selected by a catalog field `detector_runtime`, because the package's `PTDetector` asserts class indices in `{0,1,2}` and adds one, which would have stored every shark as `animal`. That was unnecessary: the package has a supported mechanism for exactly this, and Dan uses it for his own models, `MD1000-SORREL-0-0` carrying `{'model_type': 'ultralytics', 'image_size': 960, ...}`.

A model the package does not already know needs **both** halves, and neither is optional:

1. **`model_type` in the weights' embedded metadata**, written with the package's own `add_metadata_to_megadetector_model_file`. SharkTrack ships `{"metadata_format_version": 1.0, "model_version_string": "sharktrack-1.0", "model_type": "ultralytics", "image_size": 640}`.
2. **`class_mapping` in the catalog**, handed over as `--class_mapping_filename` (images) or `--class_mapping` (video). It sets `USE_MODEL_NATIVE_CLASSES` as well as the names, so the two travel together.

**Without the metadata the package decodes a YOLOv8 head through its YOLOv5 path and produces confident nonsense, with no error.** Measured on SharkTrack's own weights before stamping: categories like `19262` and confidences like `56580.871`, in a perfectly well-formed results file. This is why a re-upload of these weights must always be stamped; nothing in the app can detect that it was not.

**Never put a version-like token in a weights filename.** The package derives a model version from the filename and its version table *overrides* the file's own declared `model_type`. `sharktrack-v2.pt` matched `v2.0.0`, chose `model_type: tf` and crashed; `sharktrack-v3.pt` the same via `v3.0.0`. Every shipped filename was checked and resolves to `unknown`, which is what you want.

**The switch changed no counts.** SharkTrack's `sample.mp4` and a 30 s many-sharks clip, run through the old ultralytics loader and the new package path: identical box counts (41 and 136), track counts (7 and 7) and MaxN (3 and 3), with zero frames whose box count differed. 176 of 177 boxes agree above 0.99 IoU. The one exception is a BoT-SORT association flip where two animals cross: the same detection (confidence identical to three decimals) joined a different track, so its Kalman-smoothed position moved. Confidences differ by at most 0.001 between the two preprocessing paths, and that is enough to flip an association at a crossing. Counts, which is what the app reports, are unaffected. The package costs about 25% more per frame than ultralytics direct (20.4 vs 16.3 ms on MPS), roughly a minute per video hour against five minutes of decode.

Consequences worth knowing: SharkTrack now analyses photos as well as video, so the guard that refused them is gone; and `USE_MODEL_NATIVE_CLASSES` is a package global, safe only because every detection run is its own subprocess. A MegaDetector run in the same session still writes `1/2/3`, pinned by a regression run and by `tests/ml/test_tracking_script.py`.

**Environments.** The underwater detectors live in `env-marine` (`ml/envs/marine/`): megadetector 10.0.24, ultralytics 8.3.177, `rfdetr` 1.10.0 with `transformers` and `supervision` pinned to what it resolved on the first build, `lap`, ffmpeg 9.0.1 and opencv-python 5.0.0.93. OpenCV is pinned in the base env too: megadetector and ultralytics only set a floor, so unpinned builds silently moved from 4.x to the 5.0 major on 2026-07-02. `lap` is BoT-SORT's assignment solver; ultralytics does not depend on it and tries to pip install it the first time a tracker runs, which fails on a machine that is offline after setup. It is pinned in the base env too, since MegaDetector tracks as well, together with ffmpeg; the viewer's on-demand frame decode reads its ffmpeg from the base env (`APP_ENV` in `utils/ffmpeg_bin.py`), the one every install has. Detectors take their env from the manifest (`MegaDetectorV1000(..., env_name=manifest.env)`), as classifiers always did; there is no default.

**Categories on relabel.** The label picker sends `"animal"` beside every species label. A species label now leaves the category as the detector said it (`"elasmobranch"`, `"fish"`, `"animal"`); only a person or vehicle box (`NON_WILDLIFE_CATEGORIES` in `label_exclusion.py`) moves to the wildlife category with the label. Any category that is not person or vehicle takes the animal colour on both sides.

**MaxN frame and times.** `EventObservation.max_n_frame_number` is the frame the AI's MaxN was counted on. Its wall-clock time, and the species' first arrival in the event, are derived on request (`frame_time` in `utils/media_dates.py`, `first_arrivals` in `crud/event_observation.py`) and never stored, so the date adjustments that rewrite file times keep them right. `counts.csv` carries `max_n_frame`, `max_n_time` and `first_arrival_time` after `count`.

**Exports.** `addaxai-detections.csv`, the XLSX sheet and `addaxai-recognitions.json` carry `track_id`, the tracker's own number within the clip (`track_key`), so the recognition file reads as the run's results.json and re-ingests to the same tracks. Camtrap DP has no frame or track field; its maintainers describe a tracked animal as one media-level observation, and that is what `build_camtrap_dp_tables` writes per track: `eventStart`/`eventEnd` from the track's span through `frame_time`, the representative box, `individualID = <mediaID>_<track_key>`, `count 1`, the frames as `observationTags` (`frameStart|frameEnd|bboxFrame|frameRate`). The event row per species carries `countMethod:MaxN|maxnMediaID|maxnFrame`, the way the BRUV manuals record where a MaxN was made. GBIF ingests the event rows only, which is what the GBIF guide recommends.

Pinned by `tests/models/test_track.py`, `tests/ml/test_track_summaries.py`, `tests/ml/test_tracking_script.py`, `tests/ml/test_detection_command.py`, `tests/integration/test_tracking_ingest.py`, `tests/ml/test_detection_visibility.py`, `tests/test_crop_service.py`, `tests/services/test_video_frame_service.py`, `tests/test_max_n.py`, `tests/api/test_track_cascade.py`, `tests/api/test_file_verify_rejects.py`, `tests/api/test_file_image_frame.py`, `tests/api/test_export.py`, `tests/db/test_migration_data.py`, `tests/ml/test_detector_env.py` and `tests/test_frame_time.py`.
...
**Placement is decided by the cards, the still is the cover.** The folder a clip lands in comes from the same visible-surface rule as its cards and its row in the Files export (see "What a file is about"): its strongest track. The still beside the copy is the cover frame with the boxes on that frame drawn (`annotated_copies` draws by `frame_number == best_frame_number`, tracked or not), so a clip filed under a species whose track never crosses the cover gets a still that does not show it. That is the honest picture of the clip, not of the track; a still per track was considered and left out (nobody asked, and the cards are on the Labels page). No EXIF tags go into containers (`is_image_path`).
...
Detection itself no longer meets the bug: the tracking script decodes through ffmpeg, which reads these files fine, and its only OpenCV use is the header read plus frame 0. The Windows retry that `process_video` needed (`OPENCV_VIDEOIO_PRIORITY_FFMPEG=0`, MSMF) went with `process_video` on 2026-09-08. The upstream report ([opencv/opencv#29699](https://github.com/opencv/opencv/issues/29699), filed with a 196 KB stream-copy repro) closed with the fix above; the advisory for other wrappers is [agentmorris/MegaDetector#237](https://github.com/agentmorris/MegaDetector/issues/237). When a fixed OpenCV ships, bump the `opencv-python` pin and retest this on Windows before removing the frame-0 capture in `video_iter`.

Pinned by the frame-0 pixel assertions in `tests/ml/test_video_iter.py`. The end-to-end proof is a folder run over real Bushnell files on a Windows machine, which is how this was verified.
...
`models.json` is bundled by `backend.spec` and read by `_bundled_catalog_path()` when the remote catalog cannot be fetched. **It is a fallback, not a cache: never write the fetched catalog over it.** **The remote does not replace it, it is laid over it**: `merge_catalogs` starts from the shipped entry of every model and puts the remote entry's fields on top, remote-only models as they are. Until 2026-09-14 the remote replaced the file wholesale, so a field the code required had to exist on `main` before the code ran anywhere; a sync from `main` rewrote every installed manifest without `domain` and every video run failed at `track_filter_for`. With the merge a field this build requires is always there for the models it shipped with, the remote still adds models and updates text, and a model added remotely without a required field fails on that model alone, by name. That would turn the one file describing what this build shipped with into a copy of whatever upstream said last, and the guarantee it exists to give (an install can always name its own models) with it.
