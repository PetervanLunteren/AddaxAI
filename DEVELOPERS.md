...
**The tracker script.** `ml/inference/tracking_script.py` is the one video detection path (MegaDetector's `process_video` and its Windows crash retry are gone): every `stride`-th source frame with `stride = round(native_fps / fps)`, numbered by absolute source index as `process_video` numbered them, the detector loaded by the megadetector package, BoT-SORT over the sampled frames with SharkTrack's association parameters (match 0.97, buffer two seconds scaled to the fps, no score fusion, no ReID) and the app's own floors on the command line: a track starts at `DEFAULT_COUNTING_THRESHOLD` and keeps boxes down to `MD_OUTPUT_CONFIDENCE_THRESHOLD`, from `app/core/confidence.py` through `_build_tracking_cmd`, so the script holds no copy that could drift (`test_tracking_cmd_carries_the_apps_confidence_floors`). SharkTrack's false-positive filter (a track shorter than one second or nearly static is dropped unless its best box scored 0.7 or more) runs only for a detector whose catalog entry says `track_filter` (the four underwater detectors): on a camera trap it would delete a resting animal. The results JSON `info` records the floors and the filter. The sampling is not `process_video`'s: it turned `--time_sample 1/fps` into `int(seconds * native_fps)` and so took every 9th frame of a 29.97 fps clip at 3 fps (3.33 a second), where `round` takes every 10th. A project analysed before tracking has a denser `frames_processed` than one analysed after; nothing reads the stride back, so both ingest the same way.
