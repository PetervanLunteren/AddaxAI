...
| the track stills, the filmstrip (9) | few, far apart | `read_wanted_frames`: a seek each, then one walk for what a seek could not verify |
...
`Project.video_tracking` follows each animal through a video so the Labels page reviews one card per animal instead of one per frame. It exists for underwater footage (60 to 90 minute BRUVS drops, where the deliverable is MaxN per species) and is an option for any video. Off by default; choosing a detector whose catalog entry says `tracking_recommended` (SharkTrack, the Community Fish Detector) switches it on and lifts the frame rate to 3 fps, the tracker's default. Inference time like `video_fps`: not in the postprocessing hash, a change needs a re-analysis. The design and the decisions behind it are in `MARINE_VIDEOS_DESIGN.md`.

**One rule, extended.** The visibility rule above gains one clause:

> an untracked video detection is visible on the best frame, or verified; a tracked one is visible on its track's **representative frame** and nowhere else

A track's representative frame is the frame of its highest-confidence box (SharkTrack's choice; ties to the earliest frame), decided by `summarise_tracks` in `scoring.py`, pixel-free like `choose_frame_number`. Both frame passes (`best_frame.py` without a classifier, `classification_worker._process_video_group` with one) write that frame as a JPEG beside the best frame, same folder, same `frame{N:06d}.jpg` name, so a track whose representative is the best frame shares the file. `Track.frame_path` is set only when that JPEG exists; a frame that never decoded leaves NULL and the card renders without a picture. `on_representative_frame()` in `ml/detection_visibility.py` is a correlated EXISTS, so it drops into every query without a new join; `on_pixel_surface()` is the frames that have a JPEG (image, best frame, representative), which is what the embedding paths use. The hand copies listed in that module's docstring each carry the clause. `_video_still_for` in `embedding_utils.py` is the one resolver for the still a video box has pixels in; the crop service reads it too.

A tracked box gets no verified-anywhere exception and no best-frame card of its own: the verdict on one box reaches the whole track, so the card on the representative frame always carries it, and a sibling on the best frame is the same animal. Without that, verifying one track turned every frame it was followed through into a grid row and an export row.

**The track holds no label and no verdict.** `tracks` is one row per animal (`file_id`, `track_key` from the JSON, frame span, count, max confidence, representative frame, `frame_path`); `Detection.track_id` points at it, `ON DELETE SET NULL`. The label is the label of the boxes and the verdict is `verified` on them, one home for each fact, so every per-frame query (MaxN, exports, the video overlay) keeps working. A verdict on the card reaches the animal: `expand_to_tracks` in `crud/detection.py` turns the ids of a request into the ids of every box sharing a track, and verify, bulk verify, relabel, X, dismiss and undo all go through it. `set_file_verified` does the same for the tracks that have a box on the frame the Files viewer shows, the best frame, and touches no other track: one click on a frame with seven sharks must not sign off seven hundred. A tracked video therefore rolls up to verified only through the grid. No foreign key to a representative detection, so there is no cycle between the two tables. The purge deletes `tracks` after `detections` and before `files`.

**Seeing a track.** The card modal draws the animal's path on its still (`trackPath` in `lib/track-utils.ts`: the box centres of the track in frame order) and, under the frame line, where in the clip it was (`TrackSpan`: a bar over `File.duration_seconds`, which the ingest stores from the last processed frame at the frame rate, text only for a file analysed before that was stored). "Open in files view" hands the track along as `lbl_track`, and the Files viewer opens the clip paused on the track's representative frame with that track selected. The timeline lives in `VideoPlayer` (`TrackTimeline.tsx`), so both the Files viewer and the Counts modal have it from one component: one row per species, one bar per track in the species colour, solid when every box is verified, hatched while unverified, hollow when rejected, a playhead, and a click on a bar that seeks to the track's representative frame and selects it. A selected track keeps its box style while every other box dims. The Counts modal alone passes MaxN markers, one per species at `EventObservation.max_n_frame_number`. Selecting a track in the Files viewer selects its representative box, so X and Relabel reach the animal through the same cascade as the grid (Verify there stays the file's sign-off, as for a photo); the track row itself still holds no label and no verdict, and `trackRows` derives both from the boxes.
...
**The tracker script.** `ml/inference/tracking_script.py` replaces `process_video` when tracking is on: the same frame sampling (`stride = round(native_fps / fps)`, absolute frame numbers), the detector named by the catalog's `detector_runtime`, BoT-SORT over the sampled frames with SharkTrack's parameters (high 0.4, low 0.2, new 0.4, match 0.97, buffer two seconds scaled to the fps, no score fusion, no ReID), and SharkTrack's false-positive filter (a track shorter than one second or nearly static is dropped unless its best box scored 0.7 or more).

**It decodes through an ffmpeg pipe, not OpenCV.** On a 65 minute 4K HEVC BRUVS clip OpenCV's software decode cost 140 to 220 ms a frame in the busy parts (2.2 s per sampled frame, four and a half hours per video, with detection and tracking flat at 100 ms), and its hardware flag silently dropped back to software for whole stretches. `ffmpeg -hwaccel auto` held 8 ms a frame throughout and scales in the same pass, so the script asks it for every stride-th source frame (`select=not(mod(n,stride))`, `-fps_mode passthrough`, so output frame `i` is source frame `i * stride`) at most 1280 px wide as raw BGR: 72 ms a sample, 14 minutes for that video. Every box is normalised, so the decode size never reaches the database. ffmpeg ships in the envs as a conda-forge package like exiftool and is resolved from the detector's env by `utils/ffmpeg_bin.py`, PATH as the dev fallback. OpenCV still reads the header (fps, frame count, size). **Only boxes of a surviving track are written**, so the tracker's thresholds are the storage floor and a 90-minute reef video does not store hundreds of thousands of noise rows. Two loaders: the megadetector package for MegaDetector `.pt` and RF-DETR `.pth` files (it carries their class names), and ultralytics for a plain checkpoint such as SharkTrack, because the package forces its three classes onto any YOLO `.pt` it does not know, which would turn a shark into "animal". The script runs with `python -P`: its own directory holds the app's `megadetector.py` wrapper, which Python would otherwise find before the package; it loads the sibling `video_iter` by file path for the same reason. Progress is tqdm over every sampled frame of the run, which `_stream_process` already parses; the per-video log line says "of", not "/", because the parser reads `n/N` off any line.

**Environments.** The underwater detectors live in `env-marine` (`ml/envs/marine/`): megadetector 10.0.24, ultralytics 8.3.177, `rfdetr` 1.10.0 with `transformers` and `supervision` pinned to what it resolved on the first build, and `lap`. `lap` is BoT-SORT's assignment solver; ultralytics does not depend on it and tries to pip install it the first time a tracker runs, which fails on a machine that is offline after setup. It is pinned in the base env too, for tracking with MegaDetector, and that is the one change to the base YAML this feature makes. Detectors take their env from the manifest (`MegaDetectorV1000(..., env_name=manifest.env)`), as classifiers always did; there is no default.
...
**MaxN frame and times.** `EventObservation.max_n_frame_number` is the frame the AI's MaxN was counted on. Its wall-clock time, and the species' first arrival in the event, are derived on request (`frame_time` in `utils/media_dates.py`, `first_arrival_by_species` in `crud/event_observation.py`) and never stored, so the date adjustments that rewrite file times keep them right. `counts.csv` carries `max_n_frame`, `max_n_time` and `first_arrival_time` after `count`.

Pinned by `tests/models/test_track.py`, `tests/ml/test_track_summaries.py`, `tests/ml/test_tracking_script.py`, `tests/integration/test_tracking_ingest.py`, the track cases in `tests/ml/test_detection_visibility.py`, `tests/test_crop_service.py` and `tests/test_max_n.py`, `tests/api/test_track_cascade.py`, `tests/api/test_file_image_frame.py`, `tests/ml/test_detector_env.py` and `tests/test_frame_time.py`.
