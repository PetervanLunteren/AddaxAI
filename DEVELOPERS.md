...
> Sort the classes present in the project by taxonomy (class, order, family, genus, species, variant, name), then give rank `i` the palette entry `i mod 12`. The palette is ordered farthest-first, so consecutive entries contrast most, and consecutive ranks are the classes most likely to be confused for each other.
