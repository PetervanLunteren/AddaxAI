...
**A detector declares its classes in the catalog.** `ModelManifest.classes` (`classes` in `models.json`, on all eleven detectors) says what a detector emits: animal/person/vehicle for the MegaDetectors, `elasmobranch` for SharkTrack, `fish` for the Community Fish Detector variants. **Lookup and display only.** Nothing in the detection or ingest path reads it: a run's own JSON carries the authoritative `detection_categories`, and `json_pipeline` refuses an id that map never declared, so a stale entry can only show a wrong option in a picker and can never mislabel a stored box. It exists because nothing else in the app knows what a detector finds until a run has produced it, and the label picker needs that before the first analysis. `GET /api/projects/{id}/categories` reads it through `ManifestManager`; installed manifests pick the field up on the next catalog sync, since `write_manifest` refreshes them in place. The values were read off the weights themselves and are pinned by `tests/ml/test_catalog_schema.py`.
