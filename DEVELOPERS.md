...
**A detector declares its classes in the catalog, in one of two fields.** It exists because nothing else in the app knows what a detector finds until a run has produced it, and the label picker needs that before the first analysis. `GET /api/projects/{id}/categories` is the one reader that knows about both; installed manifests pick either field up on the next catalog sync, since `write_manifest` refreshes them in place. The values were read off the weights themselves and are pinned by `tests/ml/test_catalog_schema.py`, which also asserts that no entry declares both.

`ModelManifest.classes` is the plain list, for a detector the megadetector package names itself: animal/person/vehicle for the MegaDetectors, `fish` for the Community Fish Detector variants (RF-DETR carries its names on the detector object). **Lookup and display only.** Nothing in the detection or ingest path reads it: a run's own JSON carries the authoritative `detection_categories`, and `json_pipeline` refuses an id that map never declared, so a stale entry can only show a wrong option in a picker and can never mislabel a stored box.

`ModelManifest.class_mapping` is `{"0": "elasmobranch"}` on SharkTrack, the one detector the package cannot name, and unlike `classes` it **is** read by the detection path: it is handed over as `--class_mapping_filename`, so it decides the ids and names the run reports. The ids are the model's own, from zero. Never give one to a MegaDetector: the flag also switches the package to native class indices, which would turn its output categories from `1/2/3` into `0/1/2` and disagree with every stored `results.json` and with Timelapse. `ml/inference/class_mapping.py` writes the file for both the image and the video path, so the two cannot drift.
