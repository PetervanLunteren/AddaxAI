...
**Known gaps, deliberately open (YAGNI until reported):** on macOS the detector's own input frames stay silently colour-corrupted for such files (detection still works; everything user-visible is fixed by defence 1). On Linux the detection subprocess still dies, because the Linux opencv wheel has no other file backend to fall back to. Both wait for the upstream OpenCV fix (bug report filed with a 196 KB stream-copy repro) or a real user report.
