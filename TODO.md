...
- [ ] REVERT VALID_ANCESTOR_KEYS RESTRICTION - The `valid_ancestor_keys` parameter in `backend/app/ml/taxonomic_rollup.py` temporarily restricts rollup to only use taxonomy levels that have a label in the model's classification_categories (matching the official SpeciesNet API). This causes AddaxAI to skip 172/280 families that have no model label. Remove the `valid_ancestor_keys` parameter, the set-building block in `apply_taxonomic_rollup_to_results()`, and the filter block in `rollup_single_detection()`. Search for "TEMPORARY" comments to find all three blocks.
