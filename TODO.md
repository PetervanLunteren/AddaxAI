...
The labels page now either sorts on events or on similarity. It would be nice if there wqould be a hybrid version, where it keeps events together, but still sorts the events on similarity. What do you think? Perhaps by comnparing the centroid of each event with each other? Or would you thnk this is a feature that will not yield good results? Be honest, no sugar coating. KISS DRY YAGNI Or that we sort events also by similarity iof those are present, if not, than by what we're doing now already. Silent, no warnings, etc. What dop you think? ANd perhaps we can also aaply the projects-mode deployment-grouping tweak, so we have emb? y -> sort on similarity, n -> project? y: per deployment per event, n by event only (current appraoch). WOuld that work? We just need to think of an way to group events by simmilarity, we they are clusters of data.... How would something like that work? 
