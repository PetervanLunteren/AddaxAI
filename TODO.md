...
❯ Perfect. I will. Its downlaoding as we speak. In the meantime, lets focus on this problem. THe envs are currently getting a bit old and
  decrecated. Some newer versions of NVIDIA gpus dont run on them anymore, as the python vbersion is too old. Can we bump up all the envs to the latest stable versions so it is future proof again? Here is some more info, which are basically just todo list items that align with the general sense of updating the envs to make it more future proof. 
...
- [ ] Bump the pytorch env from Python 3.8 to 3.11 (or higher, what do you recommend, whats the latest stable python, what is best to bump it too?) (3.8 is EOL since Oct 2024 and recent torch wheels are starting to drop py38 builds). Also bump torch alongside the python jump. SpeciesNet-fine-tuned classifiers (.pt files with pickled onnx2torch operator classes) need a smoke test after the bump: load NAM-ADS-v1 or similar and confirm torch.load() succeeds across the major version jump. 
