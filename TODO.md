...
- [ ] If the location of a deployment has been changed, Or if a deployment has been moved: We get this message in the deployments page "Can't find files for 1 deployment in project_Kenya". But there is no project-wide banner or notification saying that some deployments can't be found anymore. This is probably something that we want since it's pretty important. Right.? What Do you think? 
- [ ] Double check if the GitHub actions that count the number of downloads from assets actually works on this repo. I want the action to start counting downloads from release v7.0.2 onwards. Everything below that does not matter. I do not need its counts. 
- [ ] This to-do listmd file is personal. I do not want it on the public GitHub repo. How can we make sure that it's not on the repo anymore? I already added it to the gitignore, but apparently that is not enough. 
- [ ] On the project modes process page. New Deployment Form. Would you lean against or for hiding the notes and tags section? The folder and the site are probably the most important ones. The Notes and Tags section feel like noise. What do you think? Hide it under a collapsible? Please be honest, no sugar coating. Really tell me what you think. Perhaps we can hide it under a collapsible or similar and remember the state in local storage so every time a new project opens or the user lands on the same page it is as they left it before. KISS DRY YAGNI.
- [ ] In the counts CSV export, Which is obviously also the XLX sheet, Let's add a column besides observation_type, That shows the maximum confident detection. Perhaps max conf detection (which can be animal, person, vehicle, shark, whatever the detection model is run), and a column for the max conf classification (Which can be monkey, donkey, lion, etc. Whatever the classification model predicted and has the maximum confidence in that file). Does that make sense? That way people know instantly what the single label was given to a file. People can work with that if they want to simplify detections and events, etc. If we are implementing this, make sure we also update the documentation on the exports page. Explicitly explaining what these columns do. What do you think? Be honest, no sugar coating. KISS DRY YAGNI. 
