...
- [ ] If a user has done some label cleanup and is now at the counts step, and finds out some of the labels he cares about are still wrong, how does he change them? Currently he has to go back and set filters to find the crops and the relabel them, right? How ca we improive this? Perhaps a "Show in label editor" under the kebab that opens the label cleanup step/page with all the filters reset execept event=event_id (we might need to add this filter to the options, we can hide it from user interaction, i dont expect somebody to actually select an event ID, URL is fine)? But then the user is out of the flow, and has to go back to the counts step and reset the filters to find its original process.... Or we could add label cleanup in the count modal (without adding visual features, but for example by clicking on the label it opens the label selection thing)... If for the latter, we need to think about what it dfoes with the AI sugestion of the count. I would propose that it just follows the same rules. It recomputes the AI suggestion for the count based on the new labels in the event. What do you think? Be honest, no sugar coating. 

- [ ] For the recording of annotated videos, I assume it needs to record (as in, there is no easy way to quicker do this than to play the whole video?). That is no problem, its a rare action anyways. KISS. But we can make it more obvious to the user that it is recording, and how long it still takes. Can we have the "Recoding..." with red dot larger in the center? And perhaps a pbar below with the percentage (you know the total video length / frame count, right?). How would that work? Lets first discuss before we implement. 
...
- [ ] I think we need to rethink our frames storage plan. Now that we have separated counts and labels as separate steps, you can get this problem (which is already a problem on my very limited dataset). Imagine:
              - Process videos
              - in label cleanup step we only see the crops of the best frames: http://localhost:5173/folder-runs/a8138248-449e-4874-ba09-54ad9df9be63/labels
              - in counts step we see the labels of all the frames: http://localhost:5173/folder-runs/a8138248-449e-4874-ba09-54ad9df9be63/counts
              - there is currently no way to cleanup the labels of the non-best frames, so even if you cleanup 100% of step label, you get a video count suggesiton like this: '/Users/peter/Desktop/Screenshot 2026-06-12 at 16.52.11.png'
              - it regards "1 video 21 Feb 2022, 21:53 Viewing REC0028.mp4 (1 of 1)"

            So i guess the problem with the current workflow is that we can only clean the labels if the best frames, while we use all of the labels in the count suggestion. See the problem? How can we avoid this? I guess the proper way would be to show all the labels in the label step, so a user can fix all of them, right? But how would that work in terms oif UX UI anmd the cache, saving frames, cropping from memory, saving to tmp, etc. What is the complexity here and how to best solve this problem while still retaining the quick app we have now? What is clever to do? Do a full audit and give me a well though strucutred recommendation. I am not in a hurry. Please be thorough. 

Thanks for this change! I believe it improves the camera trap analysis workflow without making it overly complex. KISS. But... we have to think about marine research too (as AddaxAI will incorporate marine models too in the future, so better already think about it). And marine research works a little bit different. Whereas camera trap make small videos of about 10 sec, it might be good enough to only show the best frame for label cleanup, for marine research there are often much longer videos. Imagine videos of 30 minutes of a baited station where 10 species of fisgh swim in and out of the frame. Marine biologists want to get 10 MaxN counts from this. If we distille a single frame from a video, let the user only cleanup the labels of this frame, and base the counts on this single frame, it will be useless for marine biologists. DO you agree? Be honest, no sugar coating. How can we avoid this? I beliebve there is nothing else than to allow label cleanup for all frames of a video, right? And then do the AI suggestion on the MaxN of each species, or am i missing something? Please give this codebase a proper audit, do spome research on how marine biologists whant their data anootated, how their data typically looks, etc. I'm not in a hurry. I would rather have you take a bit longer and more tyokens than a quick noit-well-thought answer. Be thourough.  
