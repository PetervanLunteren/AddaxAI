...
before we continue to finetune the existing modal, what do you think of the current design? Be honest, no sugar coating. '/Users/peter/Desktop/Screenshot 2026-06-10 at 09.06.31.png' Is this a good design for a quick workflow? Or a weird modal with some akward features. I want your best thoughts on UX and UI on this part of the app. How do other mature apps do it? In the camera trap space, but also in other apps where users work with clustered images/videos. Hwo do they offer the workflow to their users? Now there is still time to pivot to another workflow, before we invest too much into this workflow. Be honest. 

- [ ] Since we are now confirm the event as a whole we should probabaly show the event in a gallery style, right? So woulc we do something like a user set setting for the number of columns they want to see in the modal, (1-4 perhaps? ) so they can quickly scan the event? Then perhaps with shortcut keys up and down they can scroll though the event? ANd with mouse scroll they can zoom in the number of cols (a bit like Timelapse does, where you mouse scroll to see more cols)? We need a quick workflow for this. The least amount of button clicks. What do you propose? Lets make a plan. 

- [ ] we need to think about what the box drawing actually does with the counts. I would say: If its not changing the MaxN, it shouldnt increase the proposed counts. That is already how it works i believ, so that is great. But what to do when the uses says: "There are actually 5 zebras instead of =what the ai thinks are 3, so I'll manually add 2. Oh I'll add some boxes too BTW". SO does that then increase the count? I dont know if it should. What if we just say that the bnoxes, either generated by AI or drawn by a human act the same. SO if somebody manyually ads two zebra, then draws one box, nothing happens with the count, but if he draws another two, the count incvreases. What do you think? 

- [ ] "viewing img_0016.jpg" should be "Viewing `img_0016.jpg`" (captial and code wrap). Perhaps also add the iteration number of the event? "Viewing `img_0016.jpg` (3 of 12)".
