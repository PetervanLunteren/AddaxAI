...
❯ A completely different approach. Now its might be confusing which tools work at which gallery views focus areas. What if we always    
  have a single image in focus, but we can conrol the size of the thumbnails in the filmstrip below, and the area of this filmstrip?    
  Then we always have the focus area for all the tools etc, and also the context. What do you think?  Perhaps with a dragable divider   
  between filmstrip and focus image? 
