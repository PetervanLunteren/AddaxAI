# Script to further identify MegaDetector animal detections using the Alita classification model v3.03
...
# Latest edit by Peter van Lunteren on 15 September 2025
...
class ImageConfig:
    ...
    BUFFER: int = 0
    RESIZE_METHOD: Literal['md_crop',
                           'rescale'] = 'md_crop'  # MD crop preferred
    MD_RESAMPLE: bool = True  # True is preferred
    # The final size of the second crop, made during the augmentation step
    IMAGE_SIZE: int = 480
    # This is the size that the images will be cropped to as part of the initial bounding box crop
    CROP_SIZE: int = 600
...
class ImageTransformer():
    ...
    def load_image(self, image_path, mode='RGB'):
        try:
            ...
            print(
                f"Warning: Unable to load the image at '{image_path}'. Skipping...")
            return None
    ...
    def get_mega_crop_values(self,
        ...
        if (left < 0) and (right > img_w):
            ...
            new_left = left - (left < 0) * left - \
                (right > img_w)*(right - img_w)
            new_right = right - (left < 0) * left - \
                (right > img_w)*(right - img_w)

        if (top < 0) and (bottom > img_h):
            new_top, new_bottom = 0, final_size
        else:
            new_top = top - (top < 0) * top - \
                (bottom > img_h) * (bottom - img_h)
            new_bottom = bottom - (top < 0) * top - \
                (bottom > img_h) * (bottom - img_h)
    ...
    def get_new_scale(self,
        ...
        '''
        def clamp(n): return max(min(1, n), 0)
    ...
    def md_crop_image(self,
        ...
        if resample:
            ...
            if scale is not None:
                img_w, img_h = int(round(img_w * scale)
                                   ), int(round(img_h * scale))
                image_arr = cv2.resize(
                    image_arr, (img_w, img_h), cv2.INTER_LANCZOS4)

        left, top, right, bottom = self.get_mega_crop_values(
            bbox, img_w, img_h, size)
        cropped_arr = image_arr[top:bottom, left:right]

        # both = size, unless one dimension was too small
        crop_h, crop_w = cropped_arr.shape[:2]
...
class CustomModel(pl.LightningModule):
    def __init__(self,
        ...
        self.backbone.classifier = self.custom_head(
...
species_list = ["banded_dotterel", "banded_rail", "bellbird", "black_backed_gull", "black_billed_gull",
                "black_fronted_tern", "blackbird", "brown_creeper", "canada_goose", "cat", "chamois",
                "chicken", "cow", "crake", "deer", "dog", "dunnock", "fantail", "fernbird", "ferret",
                "finch", "fiordland_crested_penguin", "goat", "grey_warbler", "hare", "harrier", "hedgehog",
                "horse", "human", "kaka", "kakapo", "kea", "kereru", "kiwi", "little_blue_penguin",
                "long_tailed_cuckoo", "magpie", "mallard", "morepork", "mouse", "myna", "nz_falcon",
                "oystercatcher", "paradise_duck", "parakeet", "pateke", "pheasant", "pig", "pipit",
                "possum", "pukeko", "quail", "rabbit", "rat", "redpoll", "rifleman", "robin", "rosella",
                "sealion", "sheep", "shore_plover", "silvereye", "sparrow", "spurwing_plover", "starling",
                "stilt", "stoat", "swan", "tahr", "takahe", "thrush", "tieke", "tomtit", "tui", "wallaby",
                "weasel", "weka", "white_faced_heron", "wrybill", "yellow_eyed_penguin", "yellowhammer"]
