...
A video is many frames and the AI checks each one. So one clip holds many detections, and the species can change from frame to frame. One raccoon walking through a clip was read as raccoon, badger, badger, blank and opossum on five frames in a row. Per-frame guesses are noisy. This is normal.
...
Nothing is thrown away. The Detections export still has one row per box, on every frame. The Files export has one row per video, describing the single frame that stands for it.
