...
The installer only places the app. On first launch AddaxAI downloads its AI models and runtime environment, about 5 GB, into the user's own home folder at `~/AddaxAI`, and it needs at least 7 GB free on that drive. This happens once per user account, not once per machine, so on shared computers every user goes through that first-time download. It needs an internet connection that can reach huggingface.co and github.com. To spare your users the wait, run that setup from a script too, as described below.

## Pre-install the models and environments

Needs AddaxAI 7.0.8 or newer.
...
- It sets up the account that runs it, not the machine. On shared computers run it once per user, for example from a logon script. On computers that reset user profiles, the setup is gone after the reset and runs again.
