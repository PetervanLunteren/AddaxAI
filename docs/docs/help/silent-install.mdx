...
The Windows installer is an NSIS installer, so it uses the NSIS switches. The common switches from MSI (`/quiet`, `/qn`) and Inno Setup (`/silent`, `/verysilent`) do nothing here.
...
The installer only places the app. On first launch AddaxAI downloads its AI models and runtime environment, about 5 GB, into the user's own home folder at `~/AddaxAI`, and it needs at least 7 GB free on that drive. This happens once per user account, not once per machine, so on shared computers every user goes through that first-time download. There is currently no way to pre-install this part centrally. Plan for it: the first launch needs an internet connection that can reach huggingface.co and github.com, and some patience.
