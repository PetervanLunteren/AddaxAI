...
A video analysed with [tracking](./underwater-videos.mdx) on gets one card per animal the AI followed, shown at the frame where it saw that animal best. Give the card a species, tick it or reject it, and that verdict reaches every frame of the animal. You never label the same animal twice.

To watch the animal, open its card: the still shows the path it swam as a dotted line, and the Video card says when in the clip it was there. "Open in files view" then opens the clip paused on that animal, with a timeline underneath: one row per species, one bar per animal. Click a bar to jump to that animal; the others fade so you can follow the one you picked. Mark false and Relabel then act on the animal you selected, every frame of it. Verify in this viewer still signs off the frame you see, as it does for a photo; to verify one animal, use its card.
