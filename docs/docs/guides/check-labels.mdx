...
Sorting by similarity is the trick that saves the most time. Animals that look alike sit next to each other, so you get a block of opossums, then a block of deer, and your eye only has to notice the odd one out. In the screenshot above every card is an opossum except two. Their label is different, and in a different colour, so they stand out at a glance.

You can also sort per event. That keeps each event's photos together and still orders the events by similarity. If you turned off the embedding model, there is no similarity score, so sorting falls back to date order.
...
The app can point out likely mistakes for you. It pairs the AI's labels with the embedding model: for each animal it looks at its closest look-alikes, and if they mostly carry a different label, it flags that as a suggestion. A crop labelled "canis" whose look-alikes are all "domestic dog" is the classic case.
...
Checking labels answers *which species*. It does not answer *how many*, and it does not confirm any count. The dashboard tracks the two separately, which is why "labels verified" and "counts confirmed" show different numbers. The most efficient and least noisy order is labels first, then counts. See [confirm the counts](./confirm-counts.mdx).
