...
## Detections and empties

The page has two tabs. Detections shows one card per box the AI found above your detection threshold. Empties shows one card per file, photo or video, where it found nothing at all. Every file is in one of the two, and your threshold decides which one.

Use Empties to check that the AI did not miss anything. It opens sorted by folder, so one camera's files sit together and you can scan them quickly.

<AppShot name="verifyEmpties" />

Select the ones that really are empty and click Verify to sign them off in one go. Double-click a file to see it full size, where Enter verifies it and takes you to the next one you have not checked. If you do see an animal, click "Draw a box" and drag around it. The species search opens so you can name it right away, and the file moves over to Detections.

Most empty photos do have a box, the AI just scored it too low to show you. Lower the detection confidence in More filters and those photos move to Detections, where a cropped card is faster to judge than a whole frame.

Videos work a little different. The AI checks every frame of the clip, not just the one you see, so a video in Empties means it found nothing in it. AddaxAI keeps one frame per video, the one showing the most of what the AI found, and that is the frame you see here and draw on. A play marker on the tile tells you which files are videos.
