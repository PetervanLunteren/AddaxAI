...
Most empty files do have a box, the AI just scored it too low to show you. Lower the detection confidence in More filters and those files move to Detections, where a cropped card is faster to judge than a whole frame. Do this before you check them, because saying a file is empty removes those low-scoring boxes for good. You have said there is nothing there, so the app takes you at your word and the file stays empty whatever you set the threshold to later.
