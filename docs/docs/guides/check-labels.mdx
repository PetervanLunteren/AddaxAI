...
To watch a clip, press P or the play button. The clip opens at the frame you were looking at, with each frame's boxes drawn and the same timeline underneath; the animal you selected keeps its colour while the others fade. The AI only checks a video a few frames per second, but the boxes slide from one checked frame to the next so they follow the animals instead of jumping. Where the AI lost an animal for a while, its box disappears rather than gliding across the gap. Every animal leaves a short trail showing where it has been over the last few seconds. From a card in Detections, "Open in files view" lands on that animal's frame with the animal selected.
