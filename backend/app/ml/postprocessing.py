...
def _extract_capture_times_from_results(
    md_results: dict,
    deployment_folder: Path,
) -> dict[str, datetime]:
    """
    Resolve capture timestamps for every image entry in `md_results`.

    Images use the EXIF `DateTimeOriginal` MegaDetector embedded under
    `exif_metadata`. Videos go through exiftool (one batch process).
    Returns a {relative_file: datetime} mapping. Entries that cannot be
    resolved are simply absent — the smoother just won't see them in the
    sequence build.

    Mirrors `_resolve_capture_timestamp` in `app.ml.json_pipeline` but
    operates on JSON-level inputs rather than ORM rows.
    """
    from app.utils.media_dates import extract_video_dates

    video_extensions = {"mp4", "avi", "mov", "mkv", "m4v", "wmv", "flv"}
    image_paths_to_time: dict[str, datetime] = {}
    ...
    for img in md_results.get("images") or []:
        ...
        if ext in video_extensions:
            ...
            rel_for_abs[abs_path] = rel
            continue
        exif = img.get("exif_metadata")
        if exif and "DateTimeOriginal" in exif:
            try:
                image_paths_to_time[rel] = datetime.strptime(
    ...
    if video_paths:
        video_dates = extract_video_dates(video_paths)
        for abs_path, ts in video_dates.items():
            rel = rel_for_abs.get(abs_path)
            if rel:
                image_paths_to_time[rel] = ts

    return image_paths_to_time


def _build_smoother_input_from_json(
    md_results: dict,
    deployment_folder: Path,
    independence_interval: int,
) -> list[dict]:
    """
    DB-less twin of `build_smoother_input`.

    Walks the JSON's `images` block, resolves timestamps from EXIF (images)
    and exiftool (videos), then runs the same folder-bucket + time-gap
    clustering used by the main pipeline. Returns the same CCT-format list
    `smoothing_script.py` already consumes.
    """
    import uuid as _uuid

    from app.services.event_clustering import cluster_entries_into_events

    capture_times = _extract_capture_times_from_results(
        md_results, deployment_folder
    )
    if not capture_times:
        return []

    entries = list(capture_times.items())
    smoother_input: list[dict] = []
    for cluster in cluster_entries_into_events(entries, independence_interval):
        seq_id = str(_uuid.uuid4())
        for rel_path, captured_at in cluster:
            smoother_input.append(
                {
                    "file_name": rel_path,
                    "seq_id": seq_id,
...
def run_postprocessing_on_json(
    json_path: Path,
    deployment_folder: Path,
    *,
    excluded_classes: list[str] | None = None,
    taxonomic_rollup: bool = True,
    smoothing_strength: str = "normal",
    event_smoothing: bool = True,
    independence_interval: int = 7200,
    detection_threshold: float = 0.2,
    classification_model_dir: Path | None = None,
    job_id: str | None = None,
) -> None:
    """
    DB-less postprocessing for a results.json.

    Same code path as `run_postprocessing_for_deployment` but without any
    DB writes: applies label exclusion, optional taxonomic rollup, and
    optional event smoothing in place. Used by the Timelapse runner.

    Args:
        json_path: Path to the results.json that will be rewritten in place.
        deployment_folder: Folder used to resolve relative file paths in
            the JSON for video EXIF reads.
        excluded_classes: Label names the user has marked absent in the
            project area. Plumbed through the same exclusion + rollup code
            the main pipeline uses.
        taxonomic_rollup: If True, run geofence-aware rollup. Newly created
            rolled-up labels are NOT persisted to label_taxonomy (no DB).
        smoothing_strength: "mild" / "normal" / "aggressive" (only used
            when event_smoothing is True).
        event_smoothing: Whether to run the smoothing subprocess at all.
        independence_interval: Seconds between detections that start a new
            event. Defaults to 7200s (2 h), matching the main app default.
        detection_threshold: Detection confidence floor passed through to
            the smoother.
        classification_model_dir: Path to the classification model directory
            (used to find taxonomy.csv for rollup and 7-token descriptions).
        job_id: Optional job id for cancellation tracking.
    """
    with open(json_path) as f:
        md_results = json.load(f)

    # Ensure 7-token descriptions exist (no-op when already 7-token).
    descriptions = md_results.get("classification_category_descriptions", {})
    needs_rebuild = not descriptions
    if not needs_rebuild and descriptions:
        sample = next(iter(descriptions.values()), "")
        if len(sample.split(";")) != 7:
            needs_rebuild = True
    if needs_rebuild and classification_model_dir is not None:
    ...
    # Apply label exclusion in-memory (skipped when taxonomy is available;
    # rollup handles excluded species geofence-aware in that case, matching
    # the main pipeline's behavior).
    from app.ml.label_exclusion import apply_label_exclusion_to_results

    exclusion_taxonomy = None
    if classification_model_dir is not None:
        taxonomy_csv = classification_model_dir / "taxonomy.csv"
        if taxonomy_csv.exists():
            from app.ml.taxonomic_rollup import load_taxonomy_lookup

            exclusion_taxonomy = load_taxonomy_lookup(taxonomy_csv)

    apply_label_exclusion_to_results(
        md_results, excluded_classes, exclusion_taxonomy
    )

    excluded_names: frozenset[str] | None = None
    if excluded_classes:
        excluded_names = frozenset(name.lower() for name in excluded_classes)

    # Taxonomic rollup. Timelapse runs have no project, no country code,
    # so allowed_taxonomy_keys is always None — no geofence narrowing,
    # only confidence-based rollup and excluded-name redirect.
    if taxonomic_rollup and classification_model_dir is not None:
        taxonomy_csv = classification_model_dir / "taxonomy.csv"
        if taxonomy_csv.exists():
            ...
            rollup_result = apply_taxonomic_rollup_to_results(
                ...
                taxonomy_csv,
                excluded_names=excluded_names,
                allowed_taxonomy_keys=None,
            )
            md_results = rollup_result.md_results
            # No DB persistence of rolled-up entries: Timelapse runs
            # never touch label_taxonomy.

    if not event_smoothing:
        with open(json_path, "w") as f:
            json.dump(md_results, f, indent=2)
        return

    smoother_input = _build_smoother_input_from_json(
        md_results, deployment_folder, independence_interval
    )
    if smoother_input:
        logger.info(
            f"Running event-level smoothing on {len(smoother_input)} file rows "
            f"(independence_interval={independence_interval}s)"
        )
    ...
    try:
        logger.info(f"Running Timelapse smoothing subprocess for {json_path}")
        process = popen_group(
            [str(python_path), str(script_path), input_path, opts_path, output_path],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
        ...
        # Smoothing failure is not fatal: keep the rolled-up results that
        # are already in md_results so the user still gets a usable JSON.
        logger.error(
            f"Event smoothing failed for {json_path}, "
            f"writing rollup-only results: {e}",
            exc_info=True,
        )
        with open(json_path, "w") as f:
            json.dump(md_results, f, indent=2)
