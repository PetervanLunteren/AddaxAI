...
def test_places_multi_species_file_once(db, tmp_path):
    ...
    project = make_project(db, name="prim", detection_threshold=0.5)
...
def test_group_events_keeps_burst_in_one_folder(db, tmp_path):
    ...
    project = make_project(db, name="grp", detection_threshold=0.5)
    dep = make_deployment(db, project_id=project.id)
...
def test_group_events_off_splits_burst_per_file(db, tmp_path):
    ...
    project = make_project(db, name="nogrp", detection_threshold=0.5)
    dep = make_deployment(db, project_id=project.id)
    src_a = _make_source(tmp_path, "A.jpg")
    src_b = _make_source(tmp_path, "B.jpg")
    fa = make_file(
...
def test_excluding_person_drops_person_file(db, tmp_path):
    ...
    project = make_project(db, name="excl-person", detection_threshold=0.5)
    dep = make_deployment(db, project_id=project.id)
...
def test_preview_counts_one_placement(db):
    ...
    species' folder."""
    project = make_project(db, name="prev-prim", detection_threshold=0.5)
    dep = make_deployment(db, project_id=project.id)
    f = make_file(
...
def test_video_copied_as_best_frame_jpeg(db, tmp_path):
    ...
    project = make_project(db, name="vid", detection_threshold=0.5)
    dep = make_deployment(db, project_id=project.id)
...
def test_video_without_best_frame_is_skipped(db, tmp_path):
    ...
    raw container."""
    project = make_project(db, name="vid-nobf", detection_threshold=0.5)
    dep = make_deployment(db, project_id=project.id)
    f = make_file(
        db,
        deployment_id=dep.id,
...
def test_preserves_source_subfolder_under_species(db, tmp_path):
    ...
    project = make_project(db, name="subdir", detection_threshold=0.5)
...
def test_none_mode_mirrors_source_tree(db, tmp_path):
    ...
    project = make_project(db, name="mirror", detection_threshold=0.5)
    source = tmp_path / "source"
    dep = make_deployment(
        db, project_id=project.id, folder_path=str(source)
    )
...
def test_preview_drops_excluded_person(db):
    ...
    project = make_project(db, name="prev-person", detection_threshold=0.5)
    dep = make_deployment(db, project_id=project.id)
