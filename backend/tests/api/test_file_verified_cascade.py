...
def test_a_video_rolls_up_from_its_cards_only(client, db):
    """A video is its cards, one per track, the same rule every other
    surface applies.

    A track's other boxes and an untracked box have no card in the grid,
    no crop and no thumbnail, so a user cannot verify them one by one
    however long they work. Counting them here meant any such clip was
    stuck below verified forever: measured on a real database, 24 of 26
    videos could never reach it, which would have made a photo-level
    progress bar quietly wrong for anyone working with video.
    """
    ...
    track = make_track(
        db, file_id=video.id, start_frame=10, end_frame=42, frame_count=2,
        representative_frame_number=10,
    )
    visible = make_detection(
        db, file_id=video.id, confidence=0.9, frame_number=10, track_id=track.id
    )
    # Same animal, later in the clip: a sibling box, not a card.
    make_detection(
        db, file_id=video.id, confidence=0.9, frame_number=42, track_id=track.id
    )
    # Same clip, no track: visible nowhere.
    make_detection(db, file_id=video.id, confidence=0.9, frame_number=99)
    db.commit()
