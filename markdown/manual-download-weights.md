...
## Classification (species ID) models

Classification models identify species after MegaDetector locates animals. They are hosted on HuggingFace (with one exception, Colombian Amazon, which is hosted on Zenodo), and are usually not affected by the same network restrictions as GitHub. If you still have trouble, feel free to reach out at [peter@addaxdatascience.com](mailto:peter@addaxdatascience.com).

Most classification models consist of multiple files (model weights, class list, taxonomic mapping, and sometimes a pretrained backbone). You need to download **all listed files** for a given model and place them together in the same folder.
...
4. Open the `models/cls/` subfolder inside `AddaxAI_files`. If a folder with the exact model name does not already exist, create it. The folder name must match exactly, including spaces, punctuation, and special characters (e.g., `Hawaiʻi, USA - AI Puaʻa v1.0`).
...
- **Size:** 197 MB
- **Files:**
  - [`AI4GAmazonClassification_v0.0.0.ckpt`](https://zenodo.org/records/10041983/files/AI4GAmazonClassification_v0.0.0.ckpt?download=1) — hosted on Zenodo, not HuggingFace.
